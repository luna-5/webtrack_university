...
export default function InlineTeamManagement() {
  ...
  const fetchTeamMembers = async (teamId: string) => {
    try {
      ...
      if (userRole === 'system_user') {
        ...
        allMembers = [...allMembers, ...uniqueAdmins];
      }

      if (userRole === 'team_leader') {
        allMembers = allMembers.filter((member: TeamMember) => {
          const memberRole = member.user_roles?.[0]?.role;
          return memberRole !== 'team_leader' && memberRole !== 'system_user';
        });
      }
